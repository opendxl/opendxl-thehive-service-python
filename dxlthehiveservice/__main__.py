...
from .app import TheHiveService
...
if len(sys.argv) != 2:
    print("Usage: dxlthehiveservice <configuration files directory>")
    sys.exit(1)
...
logging_config_path = os.path.join(config_dir, TheHiveService.LOGGING_CONFIG_FILE)
...
with TheHiveService(sys.argv[1]) as app:
    try:
        # Run the application
        app.run()
        running = True
